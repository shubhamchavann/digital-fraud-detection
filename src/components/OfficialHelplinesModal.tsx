...
import { 
  PhoneCall, Globe, X, ExternalLink, 
  Copy, Check, LifeBuoy, AlertTriangle, Building2
} from 'lucide-react';
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 sm:p-6 border-b border-white/10 bg-slate-950/80">
            <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
              <div className="p-2 sm:p-2.5 rounded-2xl bg-rose-500/20 text-rose-400 border border-rose-500/30 shrink-0">
                <PhoneCall className="w-4 sm:w-5 h-4 sm:h-5" />
              </div>
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                  <h2 className="text-base sm:text-xl font-bold text-white font-display truncate">
                    Official Cyber Security Helplines
                  </h2>
                  <span className="px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-mono bg-emerald-950 text-emerald-300 border border-emerald-500/30 shrink-0">
                    VERIFIED DIRECTORY
                  </span>
                </div>
                <p className="text-[11px] sm:text-xs text-slate-400 truncate">
                  Direct official government emergency numbers and reporting portals.
                </p>
              </div>
            </div>
          ...
          {/* Quick Notice Banner */}
          <div className="bg-amber-950/40 px-4 sm:px-6 py-2.5 sm:py-3 border-b border-amber-500/20 flex items-start sm:items-center gap-2 text-xs text-amber-200">
            <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0 mt-0.5 sm:mt-0" />
            <span>
              <strong>Immediate Action:</strong> If money was debited, call <strong>1930 (India)</strong>, your local national helpline, or your bank immediately to freeze the transfer.
            </span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 sm:space-y-6">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
              {HELPLINE_DATA.map((c) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-2xl text-xs font-semibold whitespace-nowrap transition-all cursor-pointer ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 sm:gap-4">
              {currentCountryData.agencies.map((agency, idx) => (
                <div
                  key={idx}
                  className={`glass-panel p-4 sm:p-5 rounded-2xl border transition-all flex flex-col justify-between space-y-3.5 ${
                ...
                >
                  <div className="space-y-1.5">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <h3 className="font-bold text-white text-xs sm:text-sm font-display">
                          {agency.name}
                        </h3>
                      </div>
                      {agency.is24x7 && (
                  ...
                  <div className="pt-3 border-t border-white/5 space-y-2">
                    {agency.number && (
                      <div className="flex flex-wrap items-center justify-between gap-2 bg-slate-950/80 p-2.5 rounded-xl border border-white/5">
                        <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 bg-slate-950/80 p-2.5 rounded-xl border border-white/5">
                      ...
                      <a
                        ...
                        className="px-3 py-1 rounded-lg bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 border border-cyan-500/30 font-semibold text-xs flex items-center gap-1 shrink-0 transition-all ml-auto"
                      >
            ...
            <div className="glass-panel p-4 sm:p-5 rounded-2xl border border-cyan-500/20 space-y-3">
          ...
          <div className="p-4 border-t border-white/10 bg-slate-950/80 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-400">
            ...
            <button
              onClick={onClose}
              className="w-full sm:w-auto px-5 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-white font-medium cursor-pointer transition-colors"
            >
              Close Directory
            </button>
          </div>
