...
import { 
  ...
  Sparkles, Server, Terminal, Copy, Check, RefreshCw, 
  Download, CornerDownRight, CheckCircle2, XCircle
} from 'lucide-react';
...
export const UrlScanner: React.FC<UrlScannerProps> = ({ onScanComplete, initialResult, initialTarget }) => {
  const [urlInput, setUrlInput] = useState<string>(initialTarget || '');
      ...
      <div className="glass-panel-elevated p-5 sm:p-7 md:p-8 rounded-3xl border border-white/10 relative overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            {/* AI Toggle */}
            <div className="flex items-center gap-2 bg-slate-900/80 px-3.5 py-2 rounded-2xl border border-white/5 text-xs text-slate-300 self-start md:self-auto shrink-0">
          ...
          >
            <div className="relative flex-1">
              <div className="absolute inset-y-0 left-0 pl-3.5 sm:pl-4 flex items-center pointer-events-none text-slate-500">
                <Globe className="w-4 sm:w-5 h-4 sm:h-5 text-cyan-400/70" />
              </div>
              <input
                id="input-url-target"
                type="text"
                ...
                placeholder="Paste link (e.g. https://paypal.secure-checkpoint.xyz)..."
                disabled={isScanning}
                className="w-full pl-10 sm:pl-12 pr-10 py-3 sm:py-3.5 rounded-2xl bg-slate-950/80 border border-white/10 text-white placeholder-slate-500 text-xs sm:text-sm focus:outline-none focus:border-cyan-500/60 focus:ring-2 focus:ring-cyan-500/20 font-mono shadow-inner transition-all disabled:opacity-50"
              />
              {urlInput && (
                <button
                  type="button"
            ...
            <button
              ...
              type="submit"
              disabled={isScanning}
              className="flex items-center justify-center gap-2 px-6 sm:px-7 py-3 sm:py-3.5 rounded-2xl font-semibold text-xs sm:text-sm text-white bg-gradient-to-r from-cyan-500 via-teal-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 shadow-[0_0_20px_rgba(6,182,212,0.4)] disabled:opacity-50 cursor-pointer transition-all active:scale-98 shrink-0"
            >
              {isScanning ? (
                <>
      ...
      <AnimatePresence>
        {isScanning && (
          <motion.div
            ...
            className="glass-panel p-6 sm:p-8 rounded-3xl border border-cyan-500/30 text-center relative overflow-hidden"
          >
            <div className="max-w-md mx-auto flex flex-col items-center space-y-4 sm:space-y-5">
              {/* Radar circular sweep */}
              <div className="relative w-24 sm:w-28 h-24 sm:h-28 rounded-full border border-cyan-500/30 bg-slate-950/90 flex items-center justify-center shadow-[0_0_40px_rgba(6,182,212,0.3)]">
                ...
                <Globe className="w-8 sm:w-10 h-8 sm:h-10 text-cyan-400 animate-pulse relative z-10" />
              </div>

              <div>
                <h3 className="text-base sm:text-lg font-bold text-white font-display">Forensic Inspection Active</h3>
                <p className="text-xs font-mono text-cyan-300 mt-1 min-h-[20px] px-2">{scanStage}</p>
              </div>
      ...
      {scanResult && !isScanning && (
        ...
        >
          ...
          {(() => {
            ...
            return (
              <div className={`p-5 sm:p-7 md:p-8 rounded-3xl border transition-all ${styles.panelClass}`}>
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5 sm:gap-6">
                  ...
                  <div className="space-y-3 flex-1 min-w-0 w-full md:w-auto">
                    ...
                    <div>
                      ...
                      <p className="text-slate-300 font-mono text-xs md:text-sm break-all mt-1 bg-black/40 p-2.5 rounded-xl border border-white/5 max-w-full">
                        {scanResult.target}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3.5 sm:gap-4 bg-slate-950/70 p-3.5 sm:p-4 rounded-2xl border border-white/10 shrink-0 w-full sm:w-auto justify-center sm:justify-start">
                    ...
                    <div className="text-left space-y-0.5">
                      ...
                      <p className="text-[11px] text-slate-400">
                        {scanResult.flags.length} Red Flags
                      </p>
                    </div>
                  </div>
                </div>

                {/* Actions: Copy Report, Download JSON */}
                <div className="flex flex-wrap items-center justify-end gap-2 pt-3.5 mt-3.5 border-t border-white/10">
                  <button
          ...
          {scanResult.domainDetails && (
            <div className="glass-panel p-5 sm:p-6 rounded-3xl border border-white/10 space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 sm:gap-3">
                <div className="bg-slate-950/60 p-3 rounded-2xl border border-white/5 overflow-hidden">
                  <p className="text-[10px] text-slate-400 font-mono uppercase">Hostname</p>
                  <p className="text-xs font-mono font-bold text-white truncate mt-0.5" title={scanResult.domainDetails.hostname}>
                ...
                <div className="bg-slate-950/60 p-3 rounded-2xl border border-white/5 overflow-hidden">
                  ...
                  <p className={`text-xs font-mono font-bold mt-0.5 flex items-center gap-1 ${
                    ...
                    {scanResult.domainDetails.sslSecure ? <Lock className="w-3 h-3 shrink-0" /> : <Unlock className="w-3 h-3 shrink-0" />}
                    <span className="truncate">{scanResult.domainDetails.sslSecure ? 'HTTPS Valid' : 'Insecure HTTP'}</span>
                  </p>
                </div>
            ...
            {scanResult.flags.length === 0 ? (
              ...
              <div className="space-y-3">
                {scanResult.flags.map((flag) => (
                  <div
                    key={flag.id}
                    className="bg-slate-950/70 p-3.5 sm:p-4 rounded-2xl border border-white/5 space-y-2 hover:border-white/10 transition-colors"
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
